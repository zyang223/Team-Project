...
def calculate_enterprise_value(ticker: str, apikey: str) -> Decimal:
    ...
    fcf_dict = {f"Year {n+1}": f"${fcf/1000000:,.0f}M" if n == 0 else f"${fcf_estimates[n-1]/1000000:,.0f}M" for n in range(1, 6)}
    fcf_table = pd.DataFrame(fcf_dict.items(), columns=['Year', 'FCF'])
    ...
    fcf_table['Present Value'] = [cf / (1 + discount_rate)**n for n, cf in enumerate(fcf_estimates, start=1)]
    fcf_table['Present Value'] = fcf_table['Present Value'].apply(lambda x: f"${x/1000000:.2f}M")
    ...
    return fcf,fcf_table,present_value,terminal_value
